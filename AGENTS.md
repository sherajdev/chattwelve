# Agent Guidelines for ChatTwelve

This document provides essential instructions, commands, and coding standards for agentic coding tools operating within this repository.
...
ChatTwelve uses BetterAuth with Supabase PostgreSQL for user authentication.

### Auth Stack
- **BetterAuth**: Server-side authentication library with session management
- **Supabase**: Self-hosted PostgreSQL database for user data
- **Next.js Middleware**: Route protection for authenticated pages

### Frontend Auth Components (frontend/components/auth/)
- `auth-provider.tsx` - React context provider for session state
...
- `/api/auth/*` - BetterAuth API endpoints (handled automatically)

### Protected Routes
The main chat interface (`/`) requires authentication. Unauthenticated users are redirected to `/login`.

### Session-User Association
- Sessions are associated with authenticated users via `user_id`
- Users can list their sessions: `GET /api/session/user/{user_id}`
- System prompts support user ownership via `user_id` field

### Configuration
Add to `frontend/.env.local`:
```
DATABASE_URL=postgresql://user:pass@localhost:5432/chattwelve
```

## Sub-Agents

For implementation, specialized agents are available in `.claude/agents/`:
